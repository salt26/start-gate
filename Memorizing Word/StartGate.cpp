...
int main(){
	...
	system("shutdown -s -f -c \"3분 안에 관문을 뚫으면 타이머를 해제시켜 주지. 만약 실패한다면 컴퓨터가 종료될거야.\" -t 180");
	...
	//delay(2000);
	cout<<"3분 안에 관문을 통과하면 컴퓨터를 사용할 수 있고, 아니면 강제종료된다."<<endl;
	//delay(2000);
	/*
	...
	*/
	...
	cout<<"종목은 영단어 뜻 맞추기. 제한 시간은 3분이다."<<endl;
	cout<<c<<"문제를 모두 풀고 그 중 "<<(c/4)*3<<"개 이상 맞으면 통과한다."<<endl<<endl; 
	...
	if(result==100){
		...
		cout<<"당장 공부해라. 아니면 컴퓨터를 사용할 수 없다."<<endl;
	}else{
		cout<<"형편없군. 정답률은 "<<result<<"%이다. ("<<c<<"문제 중 "<<score<<"문제 정답)"<<endl;
		cout<<"공부를 하지 않고 컴퓨터를 사용할 수 있을 것이라 생각하는가?"<<endl;
	}
	if(w>0){
	...
	if ((c/4)*3 <= score) {
		...
		system("shutdown -a");
		cout<<endl<<"타이머를 정지시켰다. 관문을 뚫었으니 컴퓨터를 사용해도 좋다."<<endl;
		
		system("start explorer.exe");
		system("PAUSE");
		return 0;
	}
	cout<<endl<<"관문을 뚫지 못했으니 컴퓨터가 곧 종료될 것이다."<<endl;
	cout<<"컴퓨터가 종료되기 전까지 위의 단어를 공부해라."<<endl;
	cout<<"여기서 아무 키나 누르면 컴퓨터가 바로 종료된다. "; 
	system("PAUSE");
	system("shutdown -a");
...
void error(int a) {
	...
	cout<<"개발자에게 당장 연락해라. 그렇지 않으면 컴퓨터를 사용할 수 없다."<<endl;
	cout<<"컴퓨터는 곧 종료될 것이다."<<endl; 
	system("PAUSE");
	system("shutdown -a");
	system("shutdown /s /f /t 10");
} 
